...
app.get('/opera/v1/guests', (req, res) => {
  ...
  if (!guest) return res.status(404).json({ error: 'Guest not found or not in-house' });
  res.json(guest);
});
...
app.post('/opera/v1/folios/:reservationId/charges', (req, res) => {
  ...
  const db = load();
  db.folios ||= {};
  const folio = db.folios[reservationId] ||= { reservationId, window: 1, lines: [] };
...
app.post('/__seed/guest', (req, res) => {
  const { room, lastName, reservationId = 'RES-555', guestName = 'Guest', inHouse = true } = req.body || {};
  if (!room || !lastName) return res.status(400).json({ error: 'room and lastName required' });

  const db = load(); db.guests ||= {};
